...
const quickRepliesFor = (field, profile) => {
    switch (field) {
        case 'primaryGoal':
...
const questionFor = (field, profile) => {
...
const toKg = (weight, unit) => unit === 'lbs' ? weight * 0.45359237 : weight;
const toCm = (height, unit) => unit === 'ft' ? height * 30.48 : height;
const roundTo = (value, step) => Math.max(step, Math.round(value / step) * step);
const recommendNutritionTargets = (profile) => {
    ...
    const sexConstant = gender === 'male' ? 5 :
        gender === 'female' ? -161 :
            -78;
    ...
    const dailyCalories = roundTo(Math.min(5000, Math.max(1200, maintenance + calorieAdjustment)), 50);
    const proteinGrams = roundTo(kg * 1.6, 5);
    const fatsGrams = roundTo(Math.max(kg * 0.7, (dailyCalories * 0.25) / 9), 5);
    const remainingCalories = Math.max(0, dailyCalories - proteinGrams * 4 - fatsGrams * 9);
...
const nextFieldAfter = (field, updates) => {
...
const sanitizeSemanticUpdates = (raw, currentProfile) => {
    const updates = {};
    if (typeof raw.primaryGoal === 'string' && raw.primaryGoal.trim()) {
        updates.primaryGoal = raw.primaryGoal.trim().slice(0, 120);
    }
    const rawHeightUnit = raw.heightUnit === 'ft' || raw.heightUnit === 'cm'
        ? raw.heightUnit
        : undefined;
    const rawWeightUnit = raw.weightUnit === 'lbs' || raw.weightUnit === 'kg'
        ? raw.weightUnit
        : undefined;
    ...
    if (['Employed', 'Unemployed', 'Student'].includes(String(raw.employmentStatus))) {
        updates.employmentStatus = raw.employmentStatus;
    }
    if (['Remote', 'In Office', 'Hybrid'].includes(String(raw.workLocation))) {
        updates.workLocation = raw.workLocation;
    }
    if (['Set Shift', 'Asynchronous'].includes(String(raw.scheduleType))) {
        updates.scheduleType = raw.scheduleType;
    }
    const nutritionSource = String(raw.nutritionTargetsSource ?? raw.nutritionTargets ?? '').toLowerCase();
...
const fieldIsSatisfied = (field, profile) => {
    switch (field) {
        case 'primaryGoal':
            return hasValue(profile.primaryGoal);
        case 'units':
            return (['cm', 'ft'].includes(String(profile.heightUnit)) &&
        ...
        case 'carbsGrams':
        case 'fatsGrams':
            return (profile.nutritionTargetsSource === 'recommended' ||
...
router.post('/api/chawgee/onboarding', async (req, res) => {
    try {
        ...
        const expectedField = req.body?.expectedField;
...
`.trim();
        ...
        const assistantLead = typeof interpretation.assistantLead === 'string'
            ? interpretation.assistantLead.trim()
            : '';
        const intent = (['answer', 'question', 'uncertain', 'answer_and_question'].includes(String(interpretation.intent))
            ? interpretation.intent
            : 'answer');
        const extractedUpdates = interpretation.updates &&
            typeof interpretation.updates === 'object' &&
            !Array.isArray(interpretation.updates)
            ? interpretation.updates
            : {};
        let profileUpdates = sanitizeSemanticUpdates(extractedUpdates, profile);
        const preliminaryMerged = {
            ...profile,
            ...profileUpdates,
        };
        if (preliminaryMerged.nutritionTargetsSource === 'recommended' &&
        ...
        if (Object.keys(profileUpdates).length === 0) {
            if (expectedField === 'complete') {
                return res.json({
                    assistantMessage: assistantLead ||
        ...
        const nextField = expectedField === 'complete'
            ? nextMissingField(merged)
            : nextMissingField(merged, FLOW[currentIndex + 1] ?? 'complete');
        const isComplete = nextField === 'complete';
        const nextQuestion = isComplete
            ? ''
            : questionFor(nextField, merged);
        let lead = assistantLead;
        if (profileUpdates.nutritionTargetsSource === 'recommended' &&

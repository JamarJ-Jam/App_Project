...
const required = (env, name) => {
    ...
    if (!value)
        throw new ConfigurationError(`Missing required backend configuration: ${name}.`);
    return value;
};
...
const requiredHttpsUrl = (env, name) => {
...
export const getAuthConfig = (env = process.env) => {
...
export const config = Object.freeze({
    ...
    openRouterOnboardingModel: process.env.OPENROUTER_ONBOARDING_MODEL?.trim() || 'openai/gpt-oss-120b',
});
export const getDatabaseConfig = (purpose = 'runtime', env = process.env) => {
    ...
    if (caFile) {
        if (sslMode === 'disable')
            throw new ConfigurationError('DATABASE_SSL_CA_FILE requires verified TLS.');
        try {
            ca = readFileSync(caFile, 'utf8');
        }
        catch {
            throw new ConfigurationError('Unable to read DATABASE_SSL_CA_FILE.');
        }
    }
    return {
        connectionString,
